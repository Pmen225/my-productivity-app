...
@Suite("Map node and task links")
@MainActor
struct MapLinkTests {
    @Test("Converting a node creates exactly one linked task")
    func conversionCreatesOneTask() throws {
        let world = try TestWorld()
        let map = MapDocument(title: "Plan")
        world.context.insert(map)
        let node = MapNode(title: "Write the brief", map: map)
        node.estimatedMinutes = 45
        node.colourToken = ColourToken.teal.rawValue
        node.iconName = "pencil"
        node.priority = .high
        world.context.insert(node)
        try world.context.save()

        let task = MapNodeConversion.convertToTask(node, in: world.context)

        #expect(node.linkedTask?.id == task.id)
        #expect(node.isTask)
        // The task inherits the idea's identity, not a fresh default.
        #expect(task.title == "Write the brief")
        #expect(task.estimatedMinutes == 45)
        #expect(task.colourToken == ColourToken.teal.rawValue)
        #expect(task.iconName == "pencil")
        #expect(task.priority == .high)
    }

    @Test("Converting an already-linked node never creates a second task")
    func conversionIsIdempotent() throws {
        let world = try TestWorld()
        let map = MapDocument(title: "Plan")
        world.context.insert(map)
        let node = MapNode(title: "Write the brief", map: map)
        ...
        let first = MapNodeConversion.convertToTask(node, in: world.context)
        let second = MapNodeConversion.convertToTask(node, in: world.context)
        let third = MapNodeConversion.convertToTask(node, in: world.context)
    ...
    @Test("A node reflects its linked task's completion")
    func nodeMirrorsTaskCompletion() throws {
        let world = try TestWorld()
        let map = MapDocument(title: "Plan")
        world.context.insert(map)
        let node = MapNode(title: "Ship it", map: map)
        world.context.insert(node)
        try world.context.save()
